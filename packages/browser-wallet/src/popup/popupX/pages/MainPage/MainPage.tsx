...
function useTokenBalance(accountAddress: string, contractAddress: string, tokenId: string) {
...
function TokenBalance({decimals, tokenId, contractAddress, accountAddress}: TokenBalanceProps) {
    const balanceRaw = useTokenBalance(accountAddress, contractAddress, tokenId) ?? 0n;
...
function displayCcdAsEur(microCcdPerEur: Ratio, microCcd: bigint, decimals: number) {
    ...
    const eurFormatter = new Intl.NumberFormat(undefined, {style: 'currency', currency: "EUR", maximumFractionDigits: decimals});
...
function MainPage({credential}: MainPageProps) {
    ...
    const navToTransactionLog = () => nav(relativeRoutes.home.transactionLog.path.replace(":account", credential.address));
    ...
    const tokens = useFilteredTokens(credential, false);
  ...
  :
                    <><span className="heading_large">{displayCcdAsEur(microCcdPerEur, accountInfo.accountAmount.microCcdAmount, 2)}</span>
                                                    <span className="capture__main_small">{displayAsCcd(accountInfo.accountAmount.microCcdAmount)}</span></>}
            </div>
                    ...
                    <div className="main-page__tokens-list_item" onClick={() => navToTokenDetails()}>
                        ...
                        <div className="token-balance">
                            <div className="token-balance__amount">
                                ...
                                <Plant />
                                <span className="label__main">{displayAsCcd(accountInfo.accountAmount.microCcdAmount)}</span>
                            </div>
                            {microCcdPerEur === undefined ? null :

                            <div className="token-balance__exchange-rate">
                                <span className="capture__main_small">{displayCcdAsEur(microCcdPerEur, 1000000n, 6)}</span>
                                <span className="capture__main_small">{displayCcdAsEur(microCcdPerEur, accountInfo.accountAmount.microCcdAmount,6)}</span>
                            </div>}
                        </div>
                    </div>
                    {tokens.map((token) => (
                        <div className="main-page__tokens-list_item" onClick={() => navToTokenDetails()} key={`${token.contractIndex}.${token.id}`}>
                            <div className="token-icon">
                                <Img
                            ...
                            <div className="token-balance">
                                <div className="token-balance__amount">
                                    ...
                                    <span className="label__main"><TokenBalance decimals={token.metadata.decimals} contractAddress={token.contractIndex} accountAddress={credential.address} tokenId={token.id}/></span>
                                </div>
                            </div>
                        </div>
                    ))}
