...
async function viewFile(rpcClient: JsonRpcClient, fileHashHex: string) {
    ...
    const res = await rpcClient.invokeContract({
        method: `${SPONSORED_TX_CONTRACT_NAME}.getFile`,
    ...
    if (!res || res.tag === 'failure' || !res.returnValue) {
        throw new Error(
            `RPC call 'invokeContract' on method '${SPONSORED_TX_CONTRACT_NAME}.view' of contract '${SPONSORED_TX_CONTRACT_INDEX}' failed`
        );
    }
...
export default function SPONSOREDTXS(props: WalletConnectionProps) {
    ...
    return (
        <div style={blackCardStyle}>
                        ...
                        onClick={async () => {
                            try {
                                if (selectedFile !== undefined) {
                                    ...
                                    alert('Insert a valid public key');
                                }
                            } catch (err) {
            ...
            {connection && isRegisterPublicKeyPage && account && (
                <button
